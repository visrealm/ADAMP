...
/* The rate GPU software is written against: the F18A GPU is a TMS9900 core at
   100MHz with a typical instruction of 60-150ns. */
#define VDP_GPU_DEFAULT_IPS 10000000ul

static unsigned int s_scanlines = 262u;
static unsigned int s_line      = 0u;

/* GPU instructions per scanline - see vdp_bridge_gpu_budget. Never zero: zero is
   step_n's "no cap", and a loop can run before the first reset resolves this. */
static uint32_t s_gpuBudget = VDP_GPU_DEFAULT_IPS / (262ul * 60ul);
...
static void vdp_bridge_config_load(void)
{
    ...
    bool wasReset = false;
    const bool rewrite =
        pico9918_config_validate(s_config, modelMatches, VDP_CONFIG_VERSION, &wasReset);
...
/* How much GPU work one emulated scanline buys - the GPU's clock rate in the only
   unit the host has. ADAMP_GPU_IPS overrides the rate. */
static uint32_t vdp_bridge_gpu_budget(void)
{
    ...
    /* "Off" is a cap too large to pace with, not no cap: uncapped runs a program to
       completion, and one waiting on the Z80 never completes. */
    if (ips == 0ul) {
        return (uint32_t)VDP_GPU_DEFAULT_IPS;
    }
...
void vdp_bridge_reset(unsigned int scanlines)
{
    ...
    s_gpuBudget = vdp_bridge_gpu_budget();
}
...
int vdp_bridge_loop(void)
{
    ...
    /* A scanline's worth of GPU, capped so it always comes back: the caller that
       would satisfy a program waiting on the Z80 is the one blocked inside it. */
    pico9918_gpu_step_n(PICO9918_INST s_gpuBudget);
